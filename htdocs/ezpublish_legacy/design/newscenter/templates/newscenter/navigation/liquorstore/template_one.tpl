        ...
        <div class="container">
          <div class="navbar yamm no-margin no-padding">
            <ul class="nav navbar-nav no-padding">              
              ...
              {def $subitems = fetch('content', 'list', hash('parent_node_id', $item.node_id, 'class_filter_type', 'include', 'class_filter_array', array('news_category', 'product_category', 'product_country', 'website'), 'sort_by', array('priority', true()) ))}
              <li class="{if $subitems|count()}dropdown yamm-fw {/if}no-padding no-margin float-left {$menusettings.0.data_map.link_color.data_text} {$menusettings.0.data_map.link_active_color.data_text} {$menusettings.0.data_map.link_hover_color.data_text}">                  
                    ...
                    {if $subitems|count()}
                      <ul class="dropdown-menu {$menusettings.0.data_map.background_color_yammr.data_text} no-margin no-padding">
                        ...
                        {/case}
                        {case match=2}
                        {def $subclass = "col-lg-5 col-md-5 col-sm-5 col-xs-12"}
                        {/case}
                        {case match=3}
                        {def $subclass = "col-lg-4 col-md-4 col-sm-4 col-xs-12"}
                        {/case}
                        {case match=4}
                        {def $subclass = "col-lg-3 col-md-3 col-sm-3 col-xs-12"}
                        {/case}
                        {case match=5}
                        {def $subclass = "col-lg-2 col-md-2 col-sm-2 col-xs-12"}
                        {/case}
                        {/switch}
                        <li>
