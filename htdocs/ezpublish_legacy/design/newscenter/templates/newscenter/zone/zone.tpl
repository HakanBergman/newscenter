				...
				{if $blocks}
					{foreach $blocks as $block}
						...
						{def $included_file = concat('design:newscenter/widget/', $block.class_identifier, '.tpl')}
							...
							<div class="{$block_size_desktop.data_map.widget_size_class.data_text} {$block_size_tablet.data_map.widget_size_class.data_text|explode('lg')|implode('md')} {$block_size_mobile.data_map.widget_size_class.data_text|explode('lg')|implode('sm')} no-margin no-padding container-padding-left-2 container-padding-top">
								{if and($user.is_logged_in, $user.contentobject.current.parent_nodes.0|contains($#company.node_id))}
