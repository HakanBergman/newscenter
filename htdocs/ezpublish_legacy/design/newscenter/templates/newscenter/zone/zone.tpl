...
{include uri="design:newscenter/zone/zone_size.tpl" zone=$zone}
	...
	{* Print the Zone *}
			...
			<div class="{$#zone_width} {$#zone_width_tablet} {$#zone_width_phone} no-margin no-padding{if $zone.data_map.background_color.has_content} container-padding-right {$zone.data_map.background_color.data_text}{/if}">
				...
				{if $blocks}
					{foreach $blocks as $block}
						...
						{def $included_file = concat('design:newscenter/widget/', $block.class_identifier, '.tpl')}
							...
							<div class="{$block_size_desktop.data_map.widget_size_class.data_text} {$block_size_tablet.data_map.widget_size_class.data_text|explode('lg')|implode('md')} {$block_size_mobile.data_map.widget_size_class.data_text|explode('lg')|implode('sm')} no-margin no-padding container-padding-left container-padding-top">
								{if and($user.is_logged_in, $user.contentobject.current.parent_nodes.0|contains($#company.node_id))}
