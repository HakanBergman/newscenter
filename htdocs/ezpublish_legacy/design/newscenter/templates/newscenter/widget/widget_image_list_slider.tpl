      ...
      {if $block.data_map.image_one.has_content}
        <li>
            {if $block.data_map.link_one.has_content}<a href="{$block.data_map.link_one.value}" title="{$block.data_map.link_one.data_text}"{if $block.data_map.new_window_one.value} target="_blank"{/if}>{/if}
              <img class="height-100-percent img-responsive" src="/{$block.data_map.image_one.content.original.full_path}" alt="{$block.data_map.image_one.content.alternative_text}" data-description="{$block.data_map.image_one.content.alternative_text} test"></img>
