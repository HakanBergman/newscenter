			...
			{case match=2}
				...
				{def $child_nodes = fetch('content', 'tree', hash('parent_node_id', $current_node.parent.node_id, 'class_filter_type', 'include', 'class_filter_array', array('product')))}
					{if $child_nodes}
                      ...
                      <div class="col-lg-11">
                        <button type="submit" class="btn btn-primary glyphicon glyphicon-plus addtobasket" name="ActionAddToBasket" product-id="{$child.id}" product-price="{$child.price_inc_vat}"></button>
                      </div>
									</form>									
								</li>
							{/foreach}
