									...
									<div class="container-fluid">                                    
										{foreach $zones as $zone}																	
											...
											{if $zones_count|eq(0)}
												...
												{else}
													{* Normal Zone *}
													<div class="container container-margin-top-1-px {$sitesettings.0.data_map.background_color.data_text}">
														{include uri="design:newscenter/zone/zone.tpl" zone=$zone}
											...
											{else}
												...
												{else}													
													{* Normal Zone *}
													{* Verify if we had a normal zone, if we dont, we dont need to start one *}
													{if $normal_zone|not}<div class="container container-margin-top-1-px {$sitesettings.0.data_map.background_color.data_text}">{set $normal_zone = 1}{/if}
													{* Include our template *}
													{include uri="design:newscenter/zone/zone.tpl" zone=$zone}
													{* Verify when this is our last node and end the div *}
													{if $zones_count|eq($zones|count()|dec(1))}</div>{/if}
