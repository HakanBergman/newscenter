          ...
          {case match='product_list'}
            ...
            <div class="{$column_size} container-padding-top">
              ...
              {foreach $child.children as $grandchild}
                <div class="clear">
                  <h5 class="container-padding-left-4 container-padding-top-05 font-size-product-link font-weight-normal no-margin {$node.data_map.link_color.data_text} {$node.data_map.hover_color.data_text} clear">
                    {if $grandchild.class_identifier|eq('product')}{if $grandchild.data_map.image.has_content}<img class="img-responsive max-width-2-em float-left" src="/{$grandchild.data_map.image.content.original.full_path}" alt="{$grandchild.name|wash()}"></img>{/if}{if $grandchild.data_map.image.has_content|not}<img src="{'no-image.png'|ezimage('no')}" class="img-responsive thumbnail max-width-2-em float-left no-margin" alt="{$grandchild.name|wash()}"></img>{/if}{/if}
