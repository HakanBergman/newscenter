            ...
            {   
                ...
                if (empty($_COOKIE["captcha"])) {  
                    ...
                    if (!file_exists($_SERVER["DOCUMENT_ROOT"].'/design/newscenter/images/captcha/'.$security_code.'.png')) {
                        $create_image = shell_exec('echo '.$security_code.'|convert -resize 140x25! -channel RGBA -density 144 -bordercolor none -background none -pointsize 600 text:- -fill black '.$_SERVER["DOCUMENT_ROOT"].'/design/newscenter/images/captcha/'.$security_code.'.png 2>&1');
                    }                    
