            ...
            {   
                ...
                if (empty($_COOKIE["captcha"])) {  
                    ...
                    if (!file_exists($_SERVER["DOCUMENT_ROOT"].'/design/newscenter/images/captcha/'.$security_code.'.png')) {
                        $create_image = shell_exec('echo '.$security_code.'|convert -resize 400x50 -channel RGBA -density 90 -bordercolor none -background none -pointsize 100 text:- -fill black '.$_SERVER["DOCUMENT_ROOT"].'/design/newscenter/images/captcha/'.$security_code.'.png 2>&1');
                    }                    
